...
# 531
    ...
    ax1.plot(times_to_seconds(trace_MH1.times()), trace_MH1.data, color='k', linewidth=1)
    ...
    ax1.set_ylabel('DU', fontsize=12)

    ax2 = plt.subplot(gs[2], sharex=ax0)
    trace_MH2 = stream.select(channel='MH2')[0]
    ax2.plot(times_to_seconds(trace_MH2.times()), trace_MH2.data, color='k', linewidth=1)

    # ax2.set_ylim(510-60, 510+60)
    ax2.set_ylim(280, 1000)
    ax2.set_yticks([310,510,710])
    ax2.set_ylabel('DU', fontsize=12)

    ax3 = plt.subplot(gs[3], sharex=ax0)
    trace_SHZ = stream.select(channel='SHZ')[0]
    ax3.plot(times_to_seconds(trace_SHZ.times()), trace_SHZ.data, color='k', linewidth=1)

    ax3.set_yticks([330,530,730])
    ax3.set_ylim(530-300,530+300)
    ax3.set_ylabel('DU', fontsize=12)

    ax4 = plt.subplot(gs[4], sharex=ax0)
    trace_ATT = stream.select(channel='ATT')[0]
    ax4.plot(times_to_seconds(trace_ATT.times()),
      times_to_seconds(trace_ATT.data-trace_ATT.stats.starttime.timestamp), color='k', linewidth=1)

    ax4.set_ylim(-180, 820)
    ax4.set_yticks([0,360,720])
    ax4.set_ylabel('s',rotation=90, fontsize=12)

    ax4.annotate('+{} s'.format(onset.timestamp), xy=(0.01,0.9), 
      fontsize=12, horizontalalignment="left", verticalalignment="top",
      xycoords="axes fraction")
    ax4.annotate(trace_ATT.stats.station, xy=(0.99,0.1), 
      fontsize=12, horizontalalignment="right", verticalalignment="bottom",
      xycoords="axes fraction")
    ax4.set_xlabel('Seconds after arrival time', fontsize=12, labelpad=1)
    ...
    ax1.annotate('MH1', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
      verticalalignment='center')
    ax2.annotate('MH2', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
      verticalalignment='center')
    ax3.annotate('SHZ', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
      verticalalignment='center')
    ax4.annotate('ATT', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
      verticalalignment='center')

    plt.subplots_adjust(left=0.06, right=0.95, top=0.9, bottom=0.16)
    plt.savefig('../extra_plots_output/5tracks_incl_SHZ_XXXX.png')
    plt.show()
...
def five_tracks_original():
    ...
    ax2.annotate('MH2', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
      verticalalignment='center')
    ax3.annotate('AFR', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
      verticalalignment='center')
    ax4.annotate('ATT', xy=(1.01,0.5), fontsize=12,
      xycoords="axes fraction", horizontalalignment='left',
